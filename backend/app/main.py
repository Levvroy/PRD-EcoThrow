...
app = FastAPI(title="EcoThrow API", version="2.0.0")

# ── In-memory pending throws (slot eksklusif per bin) ────────────
...
async def startup():
    ...
    print("EcoThrow API v2.0 siap di http://localhost:8000")
...
@app.get("/")
async def root():
    return {"status": "ok", "message": "EcoThrow API v2.0 aktif!"}
...
async def list_users(db: AsyncSession = Depends(get_db)):
    ...
    for u in users:
        ...
        result.append({
            ...
            "level":    pts.level    if pts else "Copper",
        })
    return result
...
async def throw_event(req: ThrowReq, db: AsyncSession = Depends(get_db)):
    ...
    pts     = pts_res.scalar_one_or_none()
    if not pts:
        pts = UserPoints(user_id=user.id); db.add(pts)
    ...
    # Hapus pending throw setelah berhasil diproses
    pending_throws.pop(req.bin_id, None)
...
async def leaderboard(limit: int = 20, db: AsyncSession = Depends(get_db)):
    ...
    return [
        {
            "rank": i + 1, "name": u.name, "nim": u.nim,
            "faculty": u.faculty, "total_xp": p.total_xp,
            "level": p.level, "eco_points": p.eco_points, "streak_days": p.streak_days,
        }
...
async def leaderboard_faculty(db: AsyncSession = Depends(get_db)):
    ...
    rows = res.all()
    return [{"rank": i+1, "faculty": r.faculty, "total_xp": r.total or 0} for i, r in enumerate(rows)]
...
async def set_pending_throw(req: PendingThrowReq, db: AsyncSession = Depends(get_db)):
    ...
    if existing:
        ...
        if age <= 120:
            if existing["qr_code"] != req.user_qr_code:
                ...
                raise HTTPException(
                    409,
                    f"Bin sedang dipakai user lain. Sisa: {sisa} detik."
                )
            # User yang sama → perpanjang sesi
            pending_throws[req.bin_id]["timestamp"] = datetime.utcnow()
            return {"success": True, "message": "Sesi diperpanjang.", "renewed": True}
    ...
    return {"success": True, "message": "Pending throw terdaftar. Silakan buang sampah.", "renewed": False}
...
async def all_bins(db: AsyncSession = Depends(get_db)):
    ...
    return [
        {
            # PENTING: gunakan "id" (bukan "bin_id") — konsisten dengan semua endpoint lain
            "id":           b.id,
            "location":     b.location,
