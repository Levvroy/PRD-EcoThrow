...
// ── Tier system (Copper → Adamantium) 9 tiers ────────────────────
const TIERS = [
  { name: "Copper",     minXp: 0,      cls: "tier-copper"     },
  { name: "Bronze",     minXp: 300,    cls: "tier-bronze"     },
  { name: "Silver",     minXp: 800,    cls: "tier-silver"     },
  { name: "Gold",       minXp: 2000,   cls: "tier-gold"       },
  { name: "Platinum",   minXp: 5000,   cls: "tier-platinum"   },
  { name: "Diamond",    minXp: 10000,  cls: "tier-diamond"    },
  { name: "Mithril",    minXp: 20000,  cls: "tier-mithril"    },
  { name: "Orichalcum", minXp: 35000,  cls: "tier-orichalcum" },
  { name: "Adamantium", minXp: 60000,  cls: "tier-adamantium" },
];
...
function getTierProgress(xp) {
  ...
  if (!next) return { pct: 100, xpLeft: 0, nextName: "MAX" };
  const pct = Math.round((xp - current.minXp) / (next.minXp - current.minXp) * 100);
  return { pct, xpLeft: next.minXp - xp, nextName: next.name };
}

// ── levelBadge ─────────────────────────────────────────────────────
function levelBadge(levelOrXp) {
  let tier;
  if (typeof levelOrXp === "number") {
    tier = getTier(levelOrXp);
  } else {
    tier = TIERS.find(t => t.name === levelOrXp) || TIERS[0];
  }
  return `<span class="tier-badge ${tier.cls}">${lucideInline("shield", 11)} ${tier.name}</span>`;
}

// ── Lucide inline SVG helper ────────────────────────────────────────
// Uses the global lucide object if loaded, else falls back to text
function lucideInline(name, size = 16) {
  const icons = {
    shield: `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>`,
    star: `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>`,
    gem: `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="6 3 18 3 22 9 12 22 2 9"/><line x1="2" y1="9" x2="22" y2="9"/><line x1="12" y1="3" x2="6" y2="9"/><line x1="12" y1="3" x2="18" y2="9"/></svg>`,
    diamond: `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2.7 10.3a2.41 2.41 0 0 0 0 3.41l7.59 7.59a2.41 2.41 0 0 0 3.41 0l7.59-7.59a2.41 2.41 0 0 0 0-3.41L13.7 2.71A2.41 2.41 0 0 0 10.3 2.7Z"/></svg>`,
    ...
    wifi: `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12.55a11 11 0 0 1 14.08 0"/><path d="M1.42 9a16 16 0 0 1 21.16 0"/><path d="M8.53 16.11a6 6 0 0 1 6.95 0"/><circle cx="12" cy="20" r="1" fill="currentColor"/></svg>`,
    ...
    history: `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v5h5"/><path d="M3.05 13A9 9 0 1 0 6 5.3L3 8"/><path d="M12 7v5l4 2"/></svg>`,
    ...
    "x-circle": `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>`,
    ...
    "log-in": `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg>`,
...
function rankNum(n) {
  ...
  return `<span class="rank-pill ${cls}">${n}</span>`;
}

// ── Status badge untuk bin ─────────────────────────────────────────
function statusBadge(status) {
  const map = { PENUH: ["badge-penuh", "badge"], SEDANG: ["badge-sedang", "badge"], NORMAL: ["badge-normal", "badge"] };
  const [cls] = map[status] || ["badge-normal", "badge"];
  return `<span class="tier-badge ${cls}" style="font-size:.72rem">${status}</span>`;
}
...
function showToast(msg, type = "default", duration = 3200) {
  ...
  const icon = type === "success" ? "check-circle" : type === "error" ? "x-circle" : "zap";
  t.className = `et-toast ${type}`;
  t.innerHTML = `${lucideInline(icon, 15)} ${msg}`;
  c.appendChild(t);
...
function showLoading(el) {
  el.innerHTML = `<div class="loading-box"><div class="et-loader"></div><span>Memuat data...</span></div>`;
}
function showError(el, msg = "Gagal memuat data") {
  el.innerHTML = `<div class="et-alert et-alert-danger">${lucideInline("alert-triangle", 15)} ${msg}</div>`;
}
...
function initNavToggle() {
  const btn = document.getElementById("et-nav-toggle");
  const menu = document.getElementById("et-nav-menu");
  if (btn && menu) {
    btn.addEventListener("click", () => {
      const open = menu.classList.toggle("show");
      btn.innerHTML = open ? lucideInline("x", 20) : lucideInline("menu", 20);
    });
  }
}
